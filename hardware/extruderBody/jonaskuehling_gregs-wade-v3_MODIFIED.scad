...
/*
Some modifications made by mirage335. These are under the same license as stated above and/or the license stated in attribution.html .
Modifications include the following.
Moved filament hole, and other base holes, down 1.5mm (wade_block_depth/2-1.5).
Increased extruder base input tolerance (16+0.5).
Move glider hole (-1.5).
*/
...
module block_holes(legacy_mount=false){
	...
	translate(motor_mount_translation){
		translate([-gear_separation,0,0]){
			...
			// Filament feed.
			translate([-filament_feed_hole_offset,0,wade_block_depth/2-1.5])
			rotate([90,0,0])
			rotate(360/16)
...
*/
			...
			for (mount=[0:1]){
				translate([-filament_feed_hole_offset+25*((mount<1)?1:-1),
					-motor_mount_translation[1]-1,wade_block_depth/2-1.5])
				rotate([-90,0,0])
				rotate(360/16)
...
//%cube([12,4,5],center=true);
	...
	difference(){
		...
		//Filament Path	
		translate(idler_axis+[2+guide_height,+idler_long_side-idler_long_bottom-guide_length/2,-1.5]){
